...
#
#   - Insert clientID (4 bytes) and random number (16 bytes) in the clientHello. The 
#     random number is optional and is present only for the 2nd and subsequent URL access. 
#   - serverHello accepts the above 2 parameters and derive client-specific client-key 
#     (16 bytes) and response (16 bytes). 
#     If random is not present, it implies it's the 1st access from this client, in which
#     case, the server derives a client-specific key and return it to the client.
#     If random is present, it implies that it's the subsequent access from this client
#     in which case, the server derives the client-specific key and a response using the
#     random value, and return them to the client.
#   - The code is simplified to represent "absence" = value of 0. 
#
"""
...
class TLSConnection(TLSRecordLayer):
    ...
    def _handshakeClientAsyncHelper(self, srpParams, certParams, anonParams,
        ...
        clientHello = result
        clientHello_sv = result # save the clientHello message for later reference
        ...
        # start verifying the server response
        client_rand_in_clientHello = clientHello_sv.rand
        client_rand_in_clientHello = client_rand_in_clientHello.to_bytes(16, 'big')
        client_key_i = serverHello.client_key
        client_key_b = client_key_i.to_bytes(16, 'big')
        server_response = serverHello.response
        if DEBUG:
            print("client_rand_in_clientHello type", type(client_rand_in_clientHello), "len:", len(client_rand_in_clientHello), "val:", client_rand_in_clientHello)
            print("client_key_i type:", type(client_key_i), "value: ", client_key_i)
            print("client_key_b type:", type(client_key_b), "len:", len(client_key_b), "val: ", client_key_b)
            print("server_response", server_response)
        ckdf = ConcatKDFHMAC(algorithm=hashes.SHA256(), length=16, salt=b'0', otherinfo=client_rand_in_clientHello)
        exp_response_b = ckdf.derive(client_key_b)
        exp_response_i = int.from_bytes(exp_response_b, 'big')
        if DEBUG:
            print("expected response type", type(exp_response_b), ", len", len(exp_response_b), ", val:", exp_response_b)
        if DEBUG2:
            print("expected response:", exp_response_i)
        if server_response == exp_response_i:
            print("response matches!")
        else: 
            print("response mismatch!")
        
        # end veryfying the server response
    ...
    def _clientSendClientHello(self, settings, session, srpUsername,
        ...
        # start handling of client ID, rand 
        client_id = ((202*256+254)*256+190)*256+239 # 0xCAFEBEEF (3,405,692,655) 
        f = open("keyfile.txt", "r+")
        cKey = f.read()
        if int(cKey) == 0:
            rand = 0
            f.write(str(int(cKey)+1))
        else:
            #rand = random.randint(1, 2**32-1) # 2^32-1= 4294967295
            rand = random.randint(1, 2**128-1) # 2^128-1= 340282366920938463463374607431768211455
        f.close()
        
        if DEBUG2:
            print("sending client_id:", client_id, "rand:", rand)
        # end handling of client ID, rand
    ...
    def _handshakeServerAsyncHelper(self, verifierDB,
        ...
        (clientHello, version, cipherSuite, sig_scheme, privateKey,
            cert_chain, client_id, rand) = result
        
        # derive clientID' from clientID received in clientHello
        client_id = client_id.to_bytes(4, 'big')
        rand = rand.to_bytes(16, 'big')
        if DEBUG:
            print("client_id:", client_id, ", rand:", rand)
        # step 1: derive client specific key
        ckdf = ConcatKDFHash(algorithm=hashes.SHA256(), length=16, otherinfo=client_id)
        server_key = b'server_secret_key'
        client_key_b = ckdf.derive(server_key)
        client_key_i = int.from_bytes(client_key_b, 'big')
        if DEBUG:
            print("client_key type:", type(client_key_b), ", len", len(client_key_b), ", key:", client_key_b)
            print("client_key type:", type(client_key_i), ", key:", client_key_i)
        # step 2: derive client ID prime
        if rand == 0: # 1st time access from this client.
            response = "0" 
        else:
            ckdf = ConcatKDFHMAC(algorithm=hashes.SHA256(), length=16, salt=b'0', otherinfo=rand)
            response_b = ckdf.derive(client_key_b)
            response_i = int.from_bytes(response_b, 'big')
        if DEBUG:
            print("response type", type(response_b), ", len", len(response_b), ", response:", response_b)
            print("response type", type(response_i), ", response:", response_i)
        # end of the code to derive clientID'
        ...
        if version > (3, 3):
            for result in self._serverTLS13Handshake(settings, clientHello,
                                                     ...
                                                     version, sig_scheme,
                                                     alpn, reqCert, client_key_i, response_i):
                if result in (0, 1):
                    yield result
                else:
                    break
        ...
        serverHello.create(self.version, random, sessionID,
                           cipherSuite, client_key, response, CertificateType.x509, tackExt,
    ...
    def _serverTLS13Handshake(self, settings, clientHello, cipherSuite,
                              ...
                              srv_alpns, reqCert, client_key, response):
        ...
        serverHello.create((3, 3), getRandomBytes(32),
                           ...
                           cipherSuite, client_key, response, extensions=sh_extensions)
        ...
        if clientHello.session_id and sessionCache:
            ...
            if session:
                ...
                serverHello.create(version, getRandomBytes(32),
                                   session.sessionID, session.cipherSuite, client_key, response, 
...
#pylint: disable = undefined-loop-variable
        yield (clientHello, version, cipherSuite, sig_scheme, private_key,
               cert_chain, client_id, client_rand)
