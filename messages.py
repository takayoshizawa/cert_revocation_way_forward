...
class HandshakeMsg(object):
    ...
    def postWrite(self, w):
        ...
        if DEBUG:
            print("msg len: ", len(headerWriter.bytes + w.bytes))
            print("HandshakeMsg:postWrite result: ", headerWriter.bytes + w.bytes)
...
class ClientHello(HelloMessage):
    ...
    def create(self, version, random, session_id, cipher_suites, client_id, rand,
        ...
        self.rand = rand
        if DEBUG2:
            print("sending client_id:", self.client_id, "rand:", self.rand)
    ...
    def parse(self, p):
        ...
        if self.ssl2:
            ...
            self.client_id = p.get(4)
            self.rand = p.get(16)
            if DEBUG2:
                print("rec'd client_id:", self.client_id, "rand:", self.rand)
    ...
    def _write(self):
        ...
        if DEBUG:
            print("enter ClientHello:_write")
        w = Writer()
        ...
        if self.extensions is not None:
            ...
            w.bytes += w2.bytes
        if DEBUG:
            print("w len: ", len(w.bytes))
            print("w.bytes: ", w.bytes)
...
class ServerHello(HelloMessage):
    ...
    def create(self, version, random, session_id, cipher_suite, client_key, response,
        ...
        if DEBUG2:
            print("sending client_key:", self.client_key, "response:", self.response)
    ...
    def parse(self, p):
        ...
        self.client_key = p.get(16)
        self.response = p.get(16)
        if DEBUG2:
            print("rec'd client_key: ", self.client_key, "\nrec'd response:   ", self.response)
    ...
    def write(self):
        ...
        w.add(self.client_key, 16)
        w.add(self.response, 16)
